...
class IterationResult:
    ...
    description: str | None = None  # Human-readable description of the chart


def _build_descriptions_markdown(
    *,
    instruction: str,
    iterations: list[IterationResult],
    image_basename: str,
) -> str:
    """Build a markdown file documenting all chart iterations and their descriptions."""
    lines = [
        f"# Chart Descriptions: {image_basename}",
        "",
        "## User Instruction",
        "",
        instruction,
    ...
    for iteration in iterations:
        ...
        lines.append("")
        lines.append(f"**File:** `{iteration.chart_path.name}`")
        lines.append("")
        ...
        if iteration.feedback:
            lines.append(f"**Feedback:** {iteration.feedback}")
            lines.append("")
        elif iteration.iteration == 1:
            lines.append("**Feedback:** Initial generation (no reflection)")
            lines.append("")
    ...
    lines.extend([
        "## Final Chart",
        "",
        f"**File:** `{final_iteration.chart_path.name}`",
        "",
    ])
    
    if final_iteration.description:
        lines.append(f"**Type:** {final_iteration.description}")
        lines.append("")
    ...
    lines.extend([
        ...
        "---",
        "",
        "*Generated by the Enhanced Chart Workflow with Structured Reflection*",
    ])
...
def run_reflection_workflow(
    *,
    ...
    # sample_rows_json takes on the role of showing example rows of the dataframe to the model

    iterations: list[IterationResult] = []

    chart_v1_path = output_dir / f"{config.image_basename}_v1.png"

    description_v1, code_v1 = generate_initial_code(
    ...
    for iteration_index in range(2, max_iters + 1):
        ...
        feedback, description, candidate_code = reflect_on_chart(
    ...
    if config.save_final_code:
        ...
        # Save descriptions as a markdown file
        descriptions_path = output_dir / f"{config.image_basename}_descriptions.md"
        descriptions_content = _build_descriptions_markdown(
            instruction=resolved_instruction,
            iterations=iterations,
            image_basename=config.image_basename,
        )
        descriptions_path.write_text(descriptions_content, encoding="utf-8")
