...
# Rate limiting for arXiv API (1 request per 3 seconds recommended)
...
def arxiv_search_tool(query: str, max_results: int = 5) -> list[dict]:
    ...
    Includes automatic rate limiting to respect arXiv's API guidelines.
    """
    global _last_arxiv_request_time
    
    # Rate limiting: wait if needed
    time_since_last_request = time.time() - _last_arxiv_request_time
    if time_since_last_request < _ARXIV_RATE_LIMIT_SECONDS:
        sleep_time = _ARXIV_RATE_LIMIT_SECONDS - time_since_last_request
    ...
    try:
        ...
        _last_arxiv_request_time = time.time()  # Update timestamp after request
        response.raise_for_status()
    except requests.exceptions.HTTPError as e:
        if response.status_code == 429:
            return [{"error": "Rate limit exceeded. Please wait 30 seconds and try again. arXiv allows ~1 request per 3 seconds."}]
        return [{"error": str(e)}]
