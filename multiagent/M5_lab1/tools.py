...
def run_tools_for_step(step: dict[str, Any], ctx: dict[str, Any]) -> dict[str, Any]:
    """Execute all tools in a step sequentially, with automatic state propagation.
    
    This is the core executor for tool-based plans. It:
    1. Resolves tool arguments (including context references like "context.prod.item.price")
    2. Validates required arguments
    3. Executes each tool in sequence
    4. Auto-applies DataFrame mutations to context and DuckDB
    5. Stores results for use by subsequent tools
    
    Args:
        step: Step dict with "tools" list, each containing:
              - "use": tool name from TOOL_REGISTRY
              - "args": tool arguments (may include "xxx_from" context references)
              - "result_key": where to store result in context
        ctx: Execution context containing:
             - "__con__": DuckDB connection
             - "__frames__": {"inventory_df": DataFrame, "transaction_df": DataFrame}
             - Previous tool results (accessible via "context.result_key.field")
    
    Returns:
        Dict mapping result_key -> tool result for all tools executed in this step
        
    Raises:
        ValueError: If tool is unknown or required arguments are missing
    """
    results = {}
    
    # Execute each tool in the step sequentially
    for spec in step.get("tools", []):
        # 1. Extract tool specification
        name = spec.get("use")           # Tool name (e.g., "get_inventory_data")
        rkey = spec.get("result_key")    # Where to store result (e.g., "prod")
        
        if not name or not rkey:
            raise ValueError("Each tool spec requires 'use' and 'result_key'")
        
        # 2. Look up tool function in registry
        ...
        # Resolve context references (e.g., "price_from": "context.prod.item.price")
        args = resolve_args(raw_args, ctx)
        
        # Canonicalize argument names (e.g., "quantity" -> "qty", "unit_price" -> "price")
        ...
        # 5. Inject shared context handles (DuckDB connection and current DataFrames)
        ...
        # 6. **EXECUTE THE TOOL** - This is the heart of tool execution
        res = fn(**args)

        # 7. Auto-apply state mutations for write tools
        # Write tools (update_inventory, append_transaction) return updated DataFrames
        # in their result dict. We automatically propagate these updates to:
        # - The context (so subsequent tools see the changes)
        # - The DuckDB connection (so SQL queries see the changes)
        mutated = False
        if isinstance(res, dict):
            # Check if tool returned updated inventory DataFrame
            if "inventory_df" in res and isinstance(res["inventory_df"], pd.DataFrame):
                ctx["__frames__"]["inventory_df"] = res["inventory_df"]  # Update context
                ctx["__con__"].unregister("inventory_df")                # Remove old from DuckDB
                ctx["__con__"].register("inventory_df", res["inventory_df"])  # Register new
                mutated = True
            
            # Check if tool returned updated transaction DataFrame
            if "transaction_df" in res and isinstance(res["transaction_df"], pd.DataFrame):
                ctx["__frames__"]["transaction_df"] = res["transaction_df"]  # Update context
                ctx["__con__"].unregister("transaction_df")                  # Remove old from DuckDB
                ctx["__con__"].register("transaction_df", res["transaction_df"])  # Register new
                mutated = True

        # 8. Store result in context (for use by subsequent tools via "context.result_key")
        ctx[rkey] = res
        results[rkey] = res
        
        # 9. Mark result if state was mutated (for observability)
        if mutated:
